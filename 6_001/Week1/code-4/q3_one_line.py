...
def best_movie(movies, ratings):
    return [movie for movie,rating in zip(movies,ratings) if rating == max(ratings)][0]
...
if __name__ == "__main__":
    ...
    assert best_movie(movies, ratings) in {"Barbie", "Inception"}, f"Got best movie {best_movie(movies, ratings)}"
