...
def test1_verbatim():
    ...
    _check_pattern_results(
        practice.Verbatim("Hanson"), hanson, "hanson.match.ver.pickle"
    )
    _check_pattern_results(
        practice.Verbatim("music"), hanson, "hanson.match.ver2.pickle"
    )
...
def test2_charfrom():
    ...
    _check_pattern_results(
        practice.CharFrom("Hanson"), hanson, "hanson.match.cf.pickle"
    )
    _check_pattern_results(
        practice.CharFrom("music"), hanson, "hanson.match.cf2.pickle"
    )
...
def test4_sequence():
    p = practice.Sequence(
        [practice.CharFrom("abc"), practice.CharFrom("df"), practice.Verbatim("e")]
    )
    ...
    _check_pattern_results(
        practice.Sequence([practice.Verbatim("at "), practice.CharFrom("rstlne")]),
        hanson,
        "hanson.match.seq2.pickle",
    )


def test5_alternatives():
    p = practice.Alternatives(
        [practice.CharFrom("abc"), practice.CharFrom("df"), practice.Verbatim("e")]
    )
    ...
    assert p.match("f") == "f"
    _check_pattern_results(
        practice.Alternatives([practice.CharFrom("Hanson "), practice.Verbatim("the")]),
        hanson,
        "hanson.match.alt.pickle",
    )
...
def test6_repeat():
    ...
    _check_pattern_results(
        practice.Repeat(practice.CharFrom("abcdefghijklm"), 2, 4),
        hanson,
        "hanson.match.rep.pickle",
    )
    _check_pattern_results(
        practice.Repeat(practice.CharFrom("abcdefghijklm"), 2, 2),
        hanson,
        "hanson.match.rep2.pickle",
    )
...
def test8_star():
    ...
    assert p.match("") == ""
    assert (
        p.match("ababababababababababababababababccccccccccccdef")
        == "ababababababababababababababababcccccccccccc"
    )
    _check_pattern_results(
        practice.Star(practice.CharFrom("music")), hanson, "hanson.match.star2.pickle"
    )
...
def test_last_find_all_deffed():
    ...
    for cls in classes:
        ...
        assert find_all != None, (
            "find_all cannot be called from a %s object" % cls.__name__
        )
        methods.add(find_all)
    assert len(methods) == 1, "find_all is defined %d times (instead of once)" % len(
        methods
    )
...
def _make_patterns():
    ...
    month = practice.Alternatives(
        [
            practice.Sequence([practice.Verbatim("0"), practice.CharFrom("123456789")]),
            practice.Sequence([practice.Verbatim("1"), practice.CharFrom("012")]),
        ]
    )
    day = practice.Alternatives(
        [
            practice.Sequence([practice.Verbatim("0"), practice.CharFrom("123456789")]),
            practice.Sequence([practice.CharFrom("12"), practice.Digit()]),
            practice.Sequence([practice.CharFrom("3"), practice.CharFrom("01")]),
        ]
    )
    ...
    date = practice.Sequence([year, dash, month, dash, day])
    ...
    protocol = practice.Sequence(
        [
            practice.Alternatives(
                [practice.Verbatim("http"), practice.Verbatim("https")]
            ),
            practice.Verbatim("://"),
        ]
    )
    ...
    pathpiece = practice.Sequence(
        [
            practice.Verbatim("/"),
            practice.Star(
                practice.CharFrom(lowerletters + upperletters + digits + others)
            ),
        ]
    )
