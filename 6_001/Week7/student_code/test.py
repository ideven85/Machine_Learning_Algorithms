...
def _run_test(number):
    with open(
        os.path.join(TEST_DIRECTORY, "test_inputs", f"potluck_{number:03d}.pickle"),
        "rb",
    ) as f:
        inp = pickle.load(f)
    ...
    with open(
        os.path.join(TEST_DIRECTORY, "test_outputs", f"potluck_{number:03d}.pickle"),
        "rb",
    ) as f:
...
def test_examples():
    res1 = quiz.feed(
        {"alice": ["pickles"], "bob": ["ketchup"]}, {"pickles": 1, "ketchup": 1}
    )
    assert res1 == {"alice": "pickles", "bob": "ketchup"}

    res2 = quiz.feed(
        {"alice": ["pickles"], "bob": ["pickles"]}, {"pickles": 1, "ketchup": 1}
    )
    assert res2 is None

    res3 = quiz.feed(
        {"alice": ["pickles"], "bob": ["pickles"]}, {"pickles": 2, "ketchup": 1}
    )
    ...
    assert res5["bob"] == "pie"
    assert sorted((res5["candace"], res5["dave"], res5["emery"])) == [
        "cake",
        "cake",
        "cheese",
    ]
