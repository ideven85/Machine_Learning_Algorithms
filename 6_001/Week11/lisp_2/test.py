...
def run_continued_evaluations(ins, env=None):
    """
...
def test_global_frame_creation():
...
def test_map_filter_reduce_defined_externally_in_scheme():
    gf = lab.make_initial_frame()
    for name in MFR_NAMES:
        with pytest.raises(lab.SchemeNameError):
            lab.evaluate(name, gf)
    lab.evaluate_file(os.path.join(TEST_DIRECTORY, "test_files", "definitions.scm"), gf)
    for name in MFR_NAMES:
        with pytest.raises(lab.SchemeNameError):
            lab.evaluate(name, gf)
    lab.evaluate_file(MFR_FILE, gf)
    baseline = lab.evaluate(["lambda", ["x"], "x"])
    for name in MFR_NAMES:
        res = lab.evaluate(name, gf)
...
def test_map():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(78, gf)


def test_map_schemefunc():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(79, gf)


def test_filter():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(80, gf)


def test_filter_schemefunc():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(81, gf)


def test_reduce():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(82, gf)


def test_reduce_schemefunc():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(83, gf)


def test_map_filter_reduce():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    do_raw_continued_evaluations(84, gf)


def test_file_3():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    compare_outputs(*_test_file("small_test3.scm", 51, gf))


def test_file_4():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    compare_outputs(*_test_file("small_test4.scm", 85, gf))


def test_file_5():
    gf = lab.make_initial_frame()
    lab.evaluate_file(MFR_FILE, gf)
    compare_outputs(*_test_file("small_test5.scm", 86, gf))
...
def test_nd_mines():
    gf = lab.make_initial_frame()
    lab.evaluate_file(
        os.path.join(TEST_DIRECTORY, "test_files", "map_filter_reduce.scm"), gf
    )
    do_raw_continued_evaluations(65, gf)
...
def test_sudoku_solver():
    ...
    do_raw_continued_evaluations(66, gf)
