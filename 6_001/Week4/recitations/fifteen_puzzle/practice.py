...
def goal_test(state):
    """
    Helper function for solve fifteen_puzzle.
    Make a goal_test function that takes in a state from solve_puzzle and
    returns True if the state represents a solved fifteen_puzzle, False otherwise
    """
    return False
...
def solve_puzzle(board):
    """
    Given a fifteen_puzzle board, return the boards needed to solve the fifteen_puzzle
    move by move.
    ...
    Returns:
        ...
        move needed to solve the fifteen_puzzle. Returns None if there is no way to
        solve the fifteen_puzzle from given start board.
    """
    start = find_blank
