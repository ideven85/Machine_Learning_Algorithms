...
def binaryStrings(n):
    if n == 0: return []
    if n == 1: return ["0","1"]
    else: return [digit + bit for digit in binaryStrings(1) for bit in binaryStrings(n-1)]
...
def binaryKStrings(n,k):
    ...
    if n == 1:
        ...
        return [digit+bit for digit in binaryKStrings(1,k) for bit in binaryKStrings(n-1,k)]
