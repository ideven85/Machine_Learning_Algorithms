    ...
    return tf.keras.Sequential(
        [
            tf.keras.layers.Dense(
                fully_connected_dim, activation="relu"
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        attn_output = self.mha(
            x, x, x, mask
        )  # Self attention (batch_size, input_seq_len, fully_connected_dim)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out2, attn_weights_block2 = self.mha2(
            Q1, enc_output, enc_output, padding_mask, return_attention_scores=True
        )  # (batch_size, target_seq_len, d_model)
...
class Transformer(tf.keras.Model):
    ...
    def call(
        ...
        enc_output = self.encoder(
            input_sentence, training, enc_padding_mask
        )  # (batch_size, inp_seq_len, fully_connected_dim)
