...
def random_eulerien_graph(n):
    ...
    for v in range(n - 1):
        ...
        noeuds = random.sample(
            range(v + 1, n), random.choice(range(nb_min_neighbors, (n - v), 2))
        )
...
def is_eulerian_tour_directed(graph, tour):
    ...
    if (
        m != len(arcs) or m != required  # no arc is visited twice
    ):  # there are not more arcs in tour than in graph
        return False
...
def is_eulerian_tour_undirected(graph, tour):
    ...
    if (
        m != len(arcs) or m != required  # no edge is visited twice
    ):  # there are not more edges in tour than in graph
        return False
    # implicitly check that every arc in tour exists in the graph
