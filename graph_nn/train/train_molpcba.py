...
def train_epoch(model, optimizer, device, data_loader, epoch, LPE, batch_accumulation):
    ...
    wrapped_loss_fun = MetricWrapper(
        metric=model.loss, target_nan_mask="ignore-flatten"
    )
    ...
    for iter, (batch_graphs, batch_targets) in enumerate(data_loader):
        ...
        if LPE == "node":
            ...
            batch_scores = model.forward(
                batch_graphs, batch_x, batch_e, batch_EigVecs, batch_EigVals
            )
            ...
            batch_scores = model.forward(
                batch_graphs, batch_x, batch_e, batch_diff, batch_prod, batch_EigVals
            )
