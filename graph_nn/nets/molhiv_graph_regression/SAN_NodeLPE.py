...
class SAN_NodeLPE(nn.Module):
    def __init__(self, net_params):
        ...
        self.embedding_h = AtomEncoder(
            emb_dim=GT_hidden_dim - LPE_dim
        )  # Remove some embedding dimensions to make room for concatenating LPE
    ...
    def forward(self, g, h, e, EigVecs, EigVals):
        ...
        ).float()  # (Num nodes) x (Num Eigenvectors) x 2
        ...
        PosEnc = torch.transpose(
            PosEnc, 0, 1
        ).float()  # (Num Eigenvectors) x (Num nodes) x 2
