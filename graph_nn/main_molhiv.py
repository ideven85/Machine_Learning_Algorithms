    ...
    try:
        with tqdm(range(params["epochs"])) as t:
            for epoch in t:
                ...
                epoch_train_loss, epoch_train_auc, optimizer = train_epoch(
                    model, optimizer, device, train_loader, epoch, net_params["LPE"]
                )

                epoch_val_loss, epoch_val_auc = evaluate_network(
                    model, device, val_loader, epoch, net_params["LPE"]
                )
                _, epoch_test_auc = evaluate_network(
                    model, device, test_loader, epoch, net_params["LPE"]
                )
...
def main():
    ...
    if net_params["LPE"] not in ["node", "edge", "none"]:
        print(
            "[!] User did not provide a valid input argument for 'LPE'. Valid inputs are 'node', 'edge', and 'none'."
        )
        exit()
