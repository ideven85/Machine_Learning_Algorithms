        ...
        self.net = tf.keras.models.Sequential(
            [
                tf.keras.layers.Conv2D(
                    filters=6, kernel_size=5, activation="sigmoid", padding="same"
                ),
...
class RNNScratch(d2l.Module):
    ...
    def __init__(self, num_inputs, num_hiddens, sigma=0.01):
        super().__init__()
        self.save_hyperparameters()
        self.W_xh = tf.Variable(d2l.normal((num_inputs, num_hiddens)) * sigma)
        self.W_hh = tf.Variable(d2l.normal((num_hiddens, num_hiddens)) * sigma)
...
class RNNLMScratch(d2l.Classifier):
    ...
    def init_params(self):
        self.W_hq = tf.Variable(
            d2l.normal((self.rnn.num_hiddens, self.vocab_size)) * self.rnn.sigma
        )
...
class GRU(d2l.RNN):
    ...
    def __init__(self, num_hiddens, num_layers, dropout=0):
        ...
        self.rnn = tf.keras.layers.RNN(
            gru_cells, return_sequences=True, return_state=True, time_major=True
        )
...
def show_trace_2d(f, results):
    ...
    x1, x2 = d2l.meshgrid(d2l.arange(-5.5, 1.0, 0.1), d2l.arange(-3.0, 1.0, 0.1))
...
def train_ch11(trainer_fn, states, hyperparams, data_iter, feature_dim, num_epochs=2):
    ...
    # Initialization
    w = tf.Variable(
        tf.random.normal(shape=(feature_dim, 1), mean=0, stddev=0.01), trainable=True
    )
...
def update_D(X, Z, net_D, net_G, loss, optimizer_D):
    ...
    with tf.GradientTape() as tape:
        ...
        loss_D = (
            (loss(ones, tf.squeeze(real_Y)) + loss(zeros, tf.squeeze(fake_Y)))
            * batch_size
            / 2
        )
