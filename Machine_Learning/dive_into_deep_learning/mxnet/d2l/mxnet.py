...
class Module(d2l.nn_Module, d2l.HyperParameters):
    ...
    def plot(self, key, value, train):
        ...
        self.board.draw(
            x, d2l.numpy(value), ("train_" if train else "val_") + key, every_n=int(n)
        )
...
class LinearRegression(d2l.Module):
    ...
    def configure_optimizers(self):
        """Defined in :numref:`sec_linear_concise`"""
        return gluon.Trainer(self.collect_params(), "sgd", {"learning_rate": self.lr})
...
class FashionMNIST(d2l.DataModule):
    ...
    def __init__(self, batch_size=64, resize=(28, 28)):
        ...
        self.train = gluon.data.vision.FashionMNIST(train=True).transform_first(trans)
        self.val = gluon.data.vision.FashionMNIST(train=False).transform_first(trans)
...
class ResNeXtBlock(nn.Block):
    ...
    def __init__(
        self, num_channels, groups, bot_mul, use_1x1conv=False, strides=1, **kwargs
    ):
        super().__init__(**kwargs)
...
class RNNLMScratch(d2l.Classifier):
    ...
    def init_params(self):
        self.W_hq = d2l.randn(self.rnn.num_hiddens, self.vocab_size) * self.rnn.sigma
...
def masked_softmax(X, valid_lens):
    ...
    if valid_lens is None:
        ...
        X = npx.sequence_mask(
            X.reshape(-1, shape[-1]), valid_lens, True, value=-1e6, axis=1
        )
...
def show_trace_2d(f, results):
    ...
    x1, x2 = d2l.meshgrid(d2l.arange(-55, 1, 1), d2l.arange(-30, 1, 1))
...
def split_batch(X, y, devices):
    ...
    return (
        gluon.utils.split_and_load(X, devices),
...
def train_batch_ch13(
    net, features, labels, loss, trainer, devices, split_f=d2l.split_batch
):
...
def multibox_prior(data, sizes, ratios):
    ...
    w = (
        np.concatenate(
            (
                size_tensor * np.sqrt(ratio_tensor[0]),
    ...
    h = np.concatenate(
        (size_tensor / np.sqrt(ratio_tensor[0]), sizes[0] / np.sqrt(ratio_tensor[1:]))
    )
    # Divide by 2 to get half height and half width
    anchor_manipulations = (
        np.tile(np.stack((-w, -h, w, h)).T, (in_height * in_width, 1)) / 2
    )

    # Each center point will have `boxes_per_pixel` number of anchor boxes, so
    # generate a grid of all anchor box centers with `boxes_per_pixel` repeats
    out_grid = d2l.stack([shift_x, shift_y, shift_x, shift_y], axis=1).repeat(
        boxes_per_pixel, axis=0
    )
...
def multibox_target(anchors, labels):
    ...
    for i in range(batch_size):
        ...
        bbox_mask = np.tile(
            (np.expand_dims((anchors_bbox_map >= 0), axis=-1)), (1, 4)
        ).astype("int32")
...
class BananasDataset(gluon.data.Dataset):
    ...
    def __getitem__(self, idx):
        return (
            self.features[idx].astype("float32").transpose(2, 0, 1),
            self.labels[idx],
        )
...
def load_data_bananas(batch_size):
    ...
    val_iter = gluon.data.DataLoader(BananasDataset(is_train=False), batch_size)
...
def read_voc_images(voc_dir, is_train=True):
    ...
    for i, fname in enumerate(images):
        ...
        labels.append(
            image.imread(os.path.join(voc_dir, "SegmentationClass", f"{fname}.png"))
        )
...
class BERTEncoder(nn.Block):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pos_embedding = self.params.get(
            "pos_embedding", shape=(1, max_len, num_hiddens)
        )
...
def load_data_imdb(batch_size, num_steps=500):
    ...
    test_iter = d2l.load_array(
        (test_features, test_data[1]), batch_size, is_train=False
    )
...
def read_data_ml100k():
    ...
    data = pd.read_csv(
        os.path.join(data_dir, "u.data"), sep="\t", names=names, engine="python"
    )
...
def split_and_load_ml100k(
    split_mode="seq-aware", feedback="explicit", test_ratio=0.1, batch_size=256
):
    ...
    train_iter = gluon.data.DataLoader(
        train_set, shuffle=True, last_batch="rollover", batch_size=batch_size
    )
...
def train_recsys_rating(
    ...
    **kwargs,
    ...
    for epoch in range(num_epochs):
        metric, l = d2l.Accumulator(3), 0.0
    ...
    print(
        f"{metric[2] * num_epochs / timer.sum():.1f} examples/sec " f"on {str(devices)}"
    )
...
def hit_and_auc(rankedlist, test_matrix, k):
    hits_k = [
        (idx, val) for idx, val in enumerate(rankedlist[:k]) if val in set(test_matrix)
    ]
    hits_all = [
        (idx, val) for idx, val in enumerate(rankedlist) if val in set(test_matrix)
    ]
...
def evaluate_ranking(net, test_input, seq, candidates, num_users, num_items, devices):
    ...
    for u in range(num_users):
        ...
        for index, values in enumerate(test_data_iter):
            x = [
                gluon.utils.split_and_load(v, devices, even_split=False) for v in values
            ]
