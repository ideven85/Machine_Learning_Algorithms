...
def archs(classes):
    return [
        [Dense(input_dim=2, units=classes, activation="softmax")],
        [
...
class LossHistory(Callback):
    ...
    def plot(self, keys):
        for key in keys:
            plt.plot(
                np.arange(len(self.values[key])), np.array(self.values[key]), label=key
            )
        plt.legend()
...
def run_keras(
    ...
    if X_val is not None or split > 0:
        val_acc, val_loss = (
            history.values["epoch_val_acc"][-1],
            history.values["epoch_val_loss"][-1],
        )
        print(
...
def run_keras_2d(
    data_name, layers, epochs, display=True, split=0.25, verbose=True, trials=1
):
...
def plot_separator(ax, th, th_0):
    ...
    in_pts = []
    for p in pts:
        if (xmin - eps) <= p[0] <= (xmax + eps) and (ymin - eps) <= p[1] <= (
            ymax + eps
        ):
            duplicate = False
            for p1 in in_pts:
