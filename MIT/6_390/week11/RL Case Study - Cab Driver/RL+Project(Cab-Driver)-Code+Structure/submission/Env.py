...
class CabDriver:
    def __init__(self):
        ...
        self.action_space = [
            (p, q) for p in range(m) for q in range(m) if p != q or p == 0
        ]
        self.state_space = [
            (X, T, D) for X in range(1, m + 1) for T in range(t) for D in range(d)
        ]
    ...
    def requests(self, state):
        ...
        map_location_poisson_lambda = {1: 2, 2: 12, 3: 4, 4: 7, 5: 8}
        # if location == 0:
    ...
    def reward_func(self, state, action, Time_matrix):
        ...
        if action == (0, 0):
            ...
            # calculate reward
            reward = (R * duration_start_end) - C * (
                duration_start_end + duration_curr_start
            )
        return reward
