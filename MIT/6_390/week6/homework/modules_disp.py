...
def plot_data(data, labels, ax=None, xmin=None, xmax=None, ymin=None, ymax=None):
    # Handle 1D data
    if data.shape[0] == 1:
        data = add_ones(data)
    if ax is None:
    ...
    for yi in set([int(_y) for _y in set(labels.flatten().tolist())]):
        ...
        ax.scatter(
            data[0, cl], data[1, cl], c=color, marker=marker, s=50, edgecolors="none"
        )
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
    ax.grid(True, which="both")
...
def plot_objective_2d(
    J, xmin=-5, xmax=5, ymin=-5, ymax=5, cmin=None, cmax=None, res=50, ax=None
):
    if ax is None:
        ax = tidy_plot(xmin, xmax, ymin, ymax)
    else:
    ...
    ima = np.array(
        [
            [J(cv([x1i, x2i])) for x1i in np.linspace(xmin, xmax, res)]
