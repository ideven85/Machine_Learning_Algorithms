...
# Learn accumulator
def test_linear_accumulator(
    num_steps=10000, num_seqs=100, seq_length=5, step_size=0.01
):
...
def test_delay_num(
    delay=1, num_steps=10000, num_seqs=10000, seq_length=10, step_size=0.005
):
    # generate random training data: num_seqs of seq_length of random
    ...
    mm = rnn.sm()
    print(mm.transduce([np.array([[float(v)]]) for v in [3, 4, 5, 1, 2, -1, 6]]))
    return rnn
...
def test_delay_char(
    ...
    rnn = RNN(n, m, n, loss, f2, dLdf2, step_size, f1, df1)
    # train it
    rnn.train_seq_to_seq(data, num_steps)
    # construct a state machine and test with a (fixed) sequence
    mm = rnn.sm()
...
def test_binary_addition(
    num_seqs=1000, seq_length=5, num_steps=50000, step_size=0.01, num_hidden=8
):
    # generate random training data: num_seqs of seq_length of
    # binary integers.
    data = []
    ...
    f1 = sigmoid
    df1 = sigmoid_gradient
    loss = quadratic_loss
    f2 = lambda z: z
    dldz2 = quadratic_linear_gradient
...
def process_seq_data(words):
    alphabet = sorted(
        list(functools.reduce(lambda a, b: set(a) | set(b), words, set()))
    )
    codec = OneHotCodec(alphabet + ["."])
    data = []
    for w in words:
...
# Train rnn
def train_seq(
    data, codec, num_steps=10000, step_size=0.01, num_hidden=10, interactive=False
):
    # specify rnn
    l = codec.n
    m = num_hidden
...
def test_word(word, interactive=False, num_hidden=1, num_steps=10000, step_size=0.005):
    return generate_seq(
        [word], num_hidden=num_hidden, num_steps=num_steps, step_size=step_size
    )


# Learn models for two classes, then do classification.
def test_language(
    english=True, split=0.25, num_hidden=300, num_steps=500000, step_size=0.001
):
    if english:
...
def test_metal(
    ...
    if train == True:
        ...
        return load_seq(
            ...
            in_file,
            interactive=interactive,
            interactive_top5=interactive_top5,
            split=split,
            num_hidden=num_hidden,
            num_steps=num_steps,
            step_size=step_size,
...
def test_poem(
    interactive=False, split=0, num_hidden=80, num_steps=20000, step_size=0.001
):
...
class RNN:
    ...
    def __init__(
        ...
        self.adam = adam
        self.init_state = (
            init_state if init_state is not None else np.zeros((self.hidden_dim, 1))
        )
    ...
    def forward_propagation(self, x):
        new_state = self.f1(
            np.dot(self.Wsx, x) + np.dot(self.Wss, self.hidden_state) + self.Wss0
        )
    ...
    # With adagrad
    def sgd_step(self, xs, dLdz2s, states, gamma1=0.9, gamma2=0.999, fudge=1.0e-8):
        ...
        if self.adam:
            ...
            self.Wsx -= self.step_size * (dLdWsx_mh / (fudge + np.sqrt(dLdWsx_sqh)))
            self.Wss -= self.step_size * (dLdWss_mh / (fudge + np.sqrt(dLdWss_sqh)))
            self.Wo -= self.step_size * (dLdWo_mh / (fudge + np.sqrt(dLdWo_sqh)))
            self.Wss0 -= self.step_size * (dLdWss0_mh / (fudge + np.sqrt(dLdWss0_sqh)))
            self.Wo0 -= self.step_size * (dLdWo0_mh / (fudge + np.sqrt(dLdWo0_sqh)))
        else:
    ...
    def train_seq_to_seq(self, training_seqs, steps=100000, print_interval=None):
        if print_interval is None:
            print_interval = int(steps / 10)
        ...
        for step in range(steps):
            ...
            if (step % print_interval) == 0 and step > 0:
                print(
                    "%d/10: training error" % iters,
                    total_train_err / print_interval,
                    flush=True,
                )
                total_train_err = 0
                iters += 1
    ...
    def sm(self):
        return sm.RNN(
            self.Wsx, self.Wss, self.Wo, self.Wss0, self.Wo0, self.f1, self.f2
        )
    ...
    def gen_seq_interactive_top5(self, codec, seq_len=100, maximize=True):
        ...
        while True:
            ...
            print(
                "We recommend that you type one of the top 5 most frequent alphabets that follow '"
                + str(result)
                + "' : ",
                c_top5,
            )
