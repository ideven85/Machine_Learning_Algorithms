...
with open(file_name, "w") as f:
    for actor1, actor2, movie in acted_together_ids:
        f.write(
            str(bacon_numbers[actor1])
            + ","
            + str(bacon_numbers[actor2])
            + ","
            + str(movie_names[movie])
            + "\n"
        )
        acted_together_names[bacon_numbers[actor1]].append(
            [bacon_numbers[actor2], movie_names[movie]]
        )
        acted_together_names[bacon_numbers[actor2]].append(
            [bacon_numbers[actor1], movie_names[movie]]
        )
