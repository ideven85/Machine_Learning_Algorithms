...
def laplace_decomp(g, max_freqs):
    ...
    EigVals = torch.from_numpy(
        np.sort(np.abs(np.real(EigVals)))
...
def add_edge_laplace_feats(g):
    ...
    g.edata["diff"] = torch.abs(
        g.nodes[source].data["EigVecs"] - g.nodes[dest].data["EigVecs"]
    ).unsqueeze(2)
    g.edata["product"] = torch.mul(
        g.nodes[source].data["EigVecs"], g.nodes[dest].data["EigVecs"]
    ).unsqueeze(2)
...
class SBMsDataset(torch.utils.data.Dataset):
    ...
    def _laplace_decomp(self, max_freqs):
        self.train.graph_lists = [
            laplace_decomp(g, max_freqs) for g in self.train.graph_lists
        ]
        self.val.graph_lists = [
            laplace_decomp(g, max_freqs) for g in self.val.graph_lists
        ]
        self.test.graph_lists = [
            laplace_decomp(g, max_freqs) for g in self.test.graph_lists
        ]
    ...
    def _add_edge_laplace_feats(self):
        self.train.graph_lists = [
            add_edge_laplace_feats(g) for g in self.train.graph_lists
        ]
        self.val.graph_lists = [add_edge_laplace_feats(g) for g in self.val.graph_lists]
        self.test.graph_lists = [
            add_edge_laplace_feats(g) for g in self.test.graph_lists
        ]
