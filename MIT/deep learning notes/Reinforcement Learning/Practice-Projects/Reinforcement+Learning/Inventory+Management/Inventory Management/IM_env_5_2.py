...
class InventoryEnv:
    ...
    def transition(self, x_t_1, a_t_1, d_t):
        ...
        stock_after_sales = max(
            x_t_1[0] - d_t, 0
        )  # first this is calculated because this cannot go below 0
        stock_EOD = min(
            stock_after_sales + a_t_1, m
        )  # this is calculated second because this is added after the demand has been satisfied
    ...
    def reward(self, x_t_1, a_t_1, d_t):
        ...
        # 1. EXPECTED INCOME
        expected_income = p * min(
            d_t, x_t_1
        )  # quantity sold=d,i.e.,demand. But if d>x, then quantity sold=x,i.e.,stock from last night
        ...
        opportunity_cost = (
            profit
            * (actual_demand - demand_satisfied)
            * (actual_demand > demand_satisfied)
        )
        ...
        stock_arrived = a_t_1
        return_cost = R * (
            stock_after_sales + stock_arrived > m
        )  # can't use x_t_1 directly because it will be cut off at m
    ...
    def initial_step(self, state, action):
        assert self.action_space.contains(
            action
        )  # to check that action is a discrete value less than m
        obs = state
