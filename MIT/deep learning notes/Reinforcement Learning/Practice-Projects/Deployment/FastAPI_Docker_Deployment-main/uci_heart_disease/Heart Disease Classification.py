...
target_index = data.columns.get_loc(
    "target"
)  # Use this for separating the target labels from data
factor = 3
...
for column in columns_to_include:
    upper_lim = (
        feature_engg_data[column].mean() + feature_engg_data[column].std() * factor
    )
    lower_lim = (
        feature_engg_data[column].mean() - feature_engg_data[column].std() * factor
    )
    feature_engg_data = feature_engg_data[
        (feature_engg_data[column] < upper_lim)
        & (feature_engg_data[column] > lower_lim)
    ]
...
x, x_test, y, y_test = train_test_split(
    input_data, targets, test_size=0.1, train_size=0.9, random_state=5
)
x_train, x_valid, y_train, y_valid = train_test_split(
    x, y, test_size=0.22, train_size=0.78, random_state=5
)
...
def ml_model(classifier, classifier_name, **kwargs):
    ...
    if kwargs["x_test"] is not None:
        ...
        print("*****************************************************")
        print(
            "Model Time Complexity on Test Data: {:.3f} milli seconds".format(
                (end - start) * 1000
            )
        )
...
def plot_model_scalability_curves(
    train_sizes, training_time_mean, training_time_std, model_name
):
    """
...
def plot_model_performance_curves(
    training_time_mean, test_scores_mean, test_scores_std, model_name
):
    """
...
def generate_learning_curves(
    model, model_name, X, y, xlim=None, ylim=None, epochs=None, figsize=(20, 5)
):
    """
    ...
    train_sizes, train_scores, test_scores, training_time, _ = learning_curve(
        model,
        X,
        y,
        cv=cross_valid,
    ...
    plot_model_scalability_curves(
        train_sizes, training_time_mean, training_time_std, model_name
    )

    # Plot model performance score
    plot_model_performance_curves(
        training_time_mean, test_scores_mean, test_scores_std, model_name
    )
...
)  # Baseline model with any form of pruning
ml_model(
    DTC,
    "Decision Tree",
    x_train=x_train,
...
def DTGridSearch(X, y, param_lim):
    ...
    best_DT = GridSearchCV(
        estimator=DecisionTreeClassifier(random_state=3), param_grid=param_grid, cv=10
    )
    best_DT.fit(X, y)
    ...
    return (
        best_DT.best_params_["min_samples_leaf"],
        best_DT.best_params_["max_depth"],
        best_DT.best_params_["criterion"],
    )
...
best_min_sample_leaf, best_max_depth, best_criterion = DTGridSearch(
    x_train, y_train, (min_samples_leaf_lim, max_samples_leaf_lim, max_depth)
)
...
for i in range(1, 20):
    dec = DecisionTreeClassifier(
        criterion="gini", min_samples_leaf=12, random_state=3, max_depth=i
    )
...
tuned_DTC = DecisionTreeClassifier(
    criterion="gini", max_depth=3, min_samples_leaf=12, random_state=3
)
ml_model(
    tuned_DTC,
    "Decision Tree",
...
def NNGridSearch(X, y, param_limits):
    ...
    return (
        best_nn.best_params_["hidden_layer_sizes"],
        best_nn.best_params_["learning_rate_init"],
    )
...
best_hidden_unit, best_lr = NNGridSearch(
    x_train, y_train.values.ravel(), (hidden_units, lr)
)
...
generate_learning_curves(
    model=adaBoost, model_name="ADABoost", X=input_data, y=targets, epochs=4
)
...
def ABGridSearch(X, y, param_limits):
    ...
    best_adb = GridSearchCV(
        estimator=AdaBoostClassifier(random_state=7), param_grid=param_grid, cv=10
    )
    best_adb.fit(X, y)
    ...
    return (
        best_adb.best_params_["n_estimators"],
        best_adb.best_params_["learning_rate"],
        best_adb.best_params_["algorithm"],
    )
...
best_n_estimators, best_lr, best_algo = ABGridSearch(
    x_train, y_train.values.ravel(), (n_estimators, learning_rate, algorithm)
)
...
for i in range(5, 100, 5):
    adaBoost = AdaBoostClassifier(
        n_estimators=i, algorithm=best_algo, learning_rate=best_lr, random_state=7
    )
...
tuned_adaBoost = AdaBoostClassifier(
    n_estimators=40, algorithm=best_algo, learning_rate=best_lr, random_state=7
)
...
generate_learning_curves(
    model=tuned_adaBoost, model_name="ADABoost", X=input_data, y=targets, epochs=4
)
...
generate_learning_curves(model=svc, model_name="SVM", X=input_data, y=targets, epochs=4)
...
def SVMGridSearch(X, y, param_limits):
    ...
    svc = GridSearchCV(estimator=svm.SVC(random_state=3), param_grid=param_grid, cv=10)
...
best_C, best_gamma, best_kernel = SVMGridSearch(
    x_train, y_train.values.ravel(), (C, gammas, kernel)
)
...
generate_learning_curves(
    model=tuned_svc, model_name="SVM", X=input_data, y=targets, epochs=4
)
...
generate_learning_curves(model=knn, model_name="KNN", X=input_data, y=targets, epochs=4)
...
def KNNGridSearch(X, y, param_limits):
    ...
    return (
        ...
        knn.best_params_["algorithm"],
        knn.best_params_["leaf_size"],
    )
...
best_n_neighbors, best_algorithm, best_leaf_size = KNNGridSearch(
    x_train, y_train.values.ravel(), (n_neighbors, algorithm, leaf_size)
)
...
for i in range(1, 20):
    knn = KNeighborsClassifier(
        n_neighbors=i, leaf_size=best_leaf_size, algorithm=best_algorithm
    )
...
tuned_knn = KNeighborsClassifier(
    n_neighbors=9, leaf_size=best_leaf_size, algorithm=best_algorithm
)
...
generate_learning_curves(
    model=tuned_knn, model_name="KNN", X=input_data, y=targets, epochs=4
)
