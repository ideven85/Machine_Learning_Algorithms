...
class BaseGateway:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            error = GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, error_str
            )
    ...
    def share_files(
        ...
        if (
            self.file_share_dir
            and not self._shared_a_file
            and os.path.exists(self.file_share_dir)
        ):
    ...
    def write_submission(
        ...
        if isinstance(submission, pd.DataFrame):
            ...
            submission[desired_column_order].to_parquet(
