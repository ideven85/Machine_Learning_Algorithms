...
class TicTacToe:
    ...
    def is_winning(self, curr_state):
        ...
        # check rows
        if (
            (curr_state[0] + curr_state[1] + curr_state[2]) == 15
            or (curr_state[3] + curr_state[4] + curr_state[5]) == 15
            or (curr_state[6] + curr_state[7] + curr_state[8]) == 15
        ):
            return True

        # check cols
        if (
            (curr_state[0] + curr_state[3] + curr_state[6]) == 15
            or (curr_state[1] + curr_state[4] + curr_state[7]) == 15
            or (curr_state[2] + curr_state[5] + curr_state[8]) == 15
        ):
            return True

        # check diagonals
        if (curr_state[0] + curr_state[4] + curr_state[8]) == 15 or (
            curr_state[6] + curr_state[4] + curr_state[2]
        ) == 15:
            return True
        ...
        if terminal[1] == "Win":
            ...
            next_action = tuple(
                np.asarray(env_action_space)[
                    np.random.choice(len(env_action_space), 1)
                ][0]
            )
