"""
6.101 autocomplete_autocorrect 9:
Autocomplete
"""

#!/usr/bin/env python3
import os.path
import autocomplete_autocorrect
import json
import types
import pickle
...
def dictify(t):
    assert set(t.__dict__) == {
        "value",
        "children",
    }, "PrefixTree instances should only contain the two instance attributes mentioned in the autocomplete_autocorrect writeup."
...
# ...and back
def from_dict(d):
    t = autocomplete_autocorrect.PrefixTree()
    for k, v in d.items():
        t[k] = v
    return t
...
def test_set():
    ...
    assert dictify(t) == expect, "Your prefix tree is incorrect."
    assert (
        any_key_stored(t, ("man", "mat", "mattress", "map", "me", "met", "map")) is None
    )
    with pytest.raises(TypeError):
        t["something",] = "pam"
...
def test_big_corpora(bigtext):
    with open(
        os.path.join(TEST_DIRECTORY, "testing_data", "%s.txt" % bigtext),
        encoding="utf-8",
    ) as f:
        text = f.read()
        w = autocomplete_autocorrect.word_frequencies(text)

        w_e = read_expected("%s_words.pickle" % bigtext)

        assert w_e == dictify(w), (
            "word frequencies prefix tree does not match for %s" % bigtext
        )


def test_autocomplete_small():
    # Autocomplete on simple prefix trees with less than N valid words
    t = autocomplete_autocorrect.word_frequencies("cat car carpet")
    result = autocomplete_autocorrect.autocomplete(t, "car", 3)
    assert set(result) == {"car", "carpet"}

    t = autocomplete_autocorrect.word_frequencies("a an ant anteater a an ant a")
    ...
    t = autocomplete_autocorrect.word_frequencies("hello hell history")
    result = autocomplete_autocorrect.autocomplete(t, "help", 3)
    assert result == []
    with pytest.raises(TypeError):
        result = autocomplete_autocorrect.autocomplete(t, ("tuple",), None)
...
def test_autocomplete_big_1():
    ...
    t = autocomplete_autocorrect.word_frequencies(" ".join(word_list))
    for i in range(5):
        result1 = autocomplete_autocorrect.autocomplete(t, "ap", 1)
        result2 = autocomplete_autocorrect.autocomplete(t, "ap", 2)
        result3 = autocomplete_autocorrect.autocomplete(t, "ap", 3)
        result4 = autocomplete_autocorrect.autocomplete(t, "ap")
        result5 = autocomplete_autocorrect.autocomplete(t, "b")
...
def test_autocomplete_big_2():
    ...
    with open(
        os.path.join(TEST_DIRECTORY, "testing_data", "frankenstein.txt"),
        encoding="utf-8",
    ) as f:
        text = f.read()
    w = autocomplete_autocorrect.word_frequencies(text)
    for i in sorted(nums):
        for n in nums[i]:
            result = autocomplete_autocorrect.autocomplete(w, i, n)
    ...
    with pytest.raises(TypeError):
        result = autocomplete_autocorrect.autocomplete(w, ("tuple",), None)
...
def test_autocomplete_big_3():
    ...
    for ix in range(len(the_word) + 1):
        test = the_word[:ix]
        result = autocomplete_autocorrect.autocomplete(w, test)
...
def test_autocorrect_big():
    ...
    for i in sorted(nums):
        for n in nums[i]:
            result = autocomplete_autocorrect.autocorrect(w, i, n)
            expected = read_expected("frank_autocorrect_%s_%s.pickle" % (i, n))
            assert len(expected) == len(result), (
                ("missing" if len(result) < len(expected) else "too many")
                + " autocorrect results for "
                + repr(i)
                + " with maxcount = "
                + str(n)
            )
            assert set(expected) == set(result), (
                "autocorrect included "
                + repr(set(result) - set(expected))
                + " instead of "
                + repr(set(expected) - set(result))
                + " for "
...
def test_filter_word():
    ...
    result = autocomplete_autocorrect.word_filter(t, "1")
    assert isinstance(result, list)
    result.sort()
    assert result == [("1", 1)]

    result = autocomplete_autocorrect.word_filter(t, "me")
    assert isinstance(result, list)
    result.sort()
    assert result == [("me", 1)]

    result = autocomplete_autocorrect.word_filter(t, "mattress")
    assert isinstance(result, list)
    result.sort()
    assert result == [("mattress", 1)]

    # patterns that don't exist in the prefix tree
    result = autocomplete_autocorrect.word_filter(t, "mattresses")
    assert isinstance(result, list)
    result.sort()
    assert result == []

    result = autocomplete_autocorrect.word_filter(t, "ma")
    assert isinstance(result, list)
    result.sort()
    assert result == []
    ...
    result = autocomplete_autocorrect.word_filter(t, "wh" + "ee" * 6_000)
    assert isinstance(result, list)
    result.sort()
    assert result == []
...
def test_filter_question():
    t = autocomplete_autocorrect.word_frequencies(
        "I met a man with a mat who followed a map. The map led to more maps, 5 cats, and a bat."
    )
    # patterns that exist
    result = autocomplete_autocorrect.word_filter(t, "?")
    assert isinstance(result, list)
    ...
    result = autocomplete_autocorrect.word_filter(t, "???s")
    assert isinstance(result, list)
    result.sort()
    assert result == [("cats", 1), ("maps", 1)]

    result = autocomplete_autocorrect.word_filter(t, "??t")
    assert isinstance(result, list)
    result.sort()
    ...
    result = autocomplete_autocorrect.word_filter(t, "mat?")
    assert isinstance(result, list)
    result.sort()
    assert result == []

    result = autocomplete_autocorrect.word_filter(t, "ca?")
    assert isinstance(result, list)
    result.sort()
    assert result == []
    ...
    result = autocomplete_autocorrect.word_filter(t, "i?")
    assert isinstance(result, list)
    result.sort()
    assert result == []

    result = autocomplete_autocorrect.word_filter(t, "?o?")
    assert isinstance(result, list)
    result.sort()
    assert result == []

    result = autocomplete_autocorrect.word_filter(t, "m" + "?" * 11_000)
    assert isinstance(result, list)
    result.sort()
    assert result == []
...
def test_filter_small():
    ...
    result = autocomplete_autocorrect.word_filter(t, "m?t*")
    assert isinstance(result, list)
    result.sort()
...
def test_filter_big_1():
    ...
    t = autocomplete_autocorrect.word_frequencies(" ".join(word_list))
    for i in range(1000):
        result = autocomplete_autocorrect.word_filter(t, "ap*")
...
def test_filter_big_2():
    ...
    for ix, i in enumerate(patterns):
        result = autocomplete_autocorrect.word_filter(w, i)
