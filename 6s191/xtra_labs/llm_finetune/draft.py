...
def generate(start_text, model, tokenizer, num_steps=20, temp=1.0):
...
def generate_pt(model, tokenizer, text, num_steps=50, until=None, temp=1.0):
    device = model.device
    ...
    for name, child in module.named_children():
        if isinstance(child, layer_type):
            ...
            print(
                ...
                child.in_features,
                child.out_features,
                (child.in_features * 8 + child.out_features * 8 == num_params),
            )
    ...
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            setattr(
                ...
                name,
                LoRALinear(
                    child.in_features, child.out_features, child.weight, child.bias
                ),
            )
        else:
...
args = transformers.TrainingArguments(
    "/home/dnori/introtodeeplearning/xtra_labs/llm_finetune/outputs",
...
class PrinterCallback(transformers.TrainerCallback):
    def on_log(self, args, state, control, model, logs=None, **kwargs):
        start_text = (
            "### Human: When the weather is sunny, what color is the sky?### Assistant:"
        )
