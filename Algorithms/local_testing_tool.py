...
def RunCase(test_case):
    ...
    # N random numbers.
    b = PickFromPool(n, random.choices(range(MIN_AB, MAX_AB + 1), k=2 * n), a_as_set)
...
def AssertRaisesError(expected_message, fn, *args, **kwargs):
    try:
        fn(*args, **kwargs)
        assert False, "Expected error [{}], but there was none".format(expected_message)
...
def ReadInts(
    line, inclusive_min_n, inclusive_max_n, inclusive_min_value, inclusive_max_value
):
    if len(line) > max(
        MAX_LINE_LENGTH,
        inclusive_max_n
    ...
    out_of_bounds = list(
        filter(lambda x: not (inclusive_min_value <= x <= inclusive_max_value), ints)
    )
    if out_of_bounds:
...
def main():
    try:
        ...
        print(("Invalid input: {}".format(error))[:1000], file=sys.stderr)
        sys.exit(1)
