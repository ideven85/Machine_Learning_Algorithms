...
def gamestat_record(gamestat: GameStat, metadata: Metadata) -> str:
    ...
    assert (
        len(record_text) == metadata.reclen
    ), f"Got {len(record_text)} Should Be {metadata.reclen}"
    return record_text


# An application of the game statistics definitions.
with (Path.cwd() / "data" / "ch10_blackjack.file").open(
    "w", encoding="cp037", newline=""
) as target:
...
def display_decode(
    data: bytes, metadata: "XMetadata", field_metadata: "XField"
) -> Decimal:
...
def display_encode(
    data: Decimal, metadata: "XMetadata", field_metadata: "XField"
) -> bytes:
