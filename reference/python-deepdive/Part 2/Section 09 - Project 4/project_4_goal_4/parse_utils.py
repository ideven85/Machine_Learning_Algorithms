...
def iter_combined_plain_tuple(fnames, class_names, parsers, compress_fields):
    ...
    zipped_tuples = zip(
        *(
            iter_file(fname, class_name, parser)
            for fname, class_name, parser in zip(fnames, class_names, parsers)
        )
    )

    merged_iter = (
        itertools.chain.from_iterable(zipped_tuple) for zipped_tuple in zipped_tuples
    )
...
def group_data(fnames, class_names, parsers, compress_fields, filter_key, group_key):
    data = filtered_iter_combined(
        fnames, class_names, parsers, compress_fields, key=filter_key
    )
